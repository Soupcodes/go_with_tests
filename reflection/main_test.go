...
func TestWalk(t *testing.T) {
	...
	t.Run("with channels", func(t *testing.T) {
		...
		go func() {
			aChan <- Profile{33, "Tokyo"}
			aChan <- Profile{34, "Berlin"}
			aChan <- Profile{35, "Nairobi"}
			close(aChan)
		}()

		var got []string
		want := []string{"Tokyo", "Berlin", "Nairobi"}
	...
	t.Run("with a function", func(t *testing.T) {
		aFunc := func() (Profile, Profile) {
			return Profile{33, "Tokyo"}, Profile{34, "Berlin"}
		}

		var got []string
		want := []string{"Tokyo", "Berlin"}
